...
import { login as apiLogin, profile as apiProfile, setToken } from '../services/auth';
...
export const LoginModal: React.FC<LoginModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // 2) Guardar token para futuras llamadas (Authorization Bearer)
      const token: string | undefined = res?.data?.token;
      if (token) setToken(token);

      // 3) Tomar los datos del usuario del login; si quieres, refina con /auth/profile
      let u = res?.data?.user;
      try {
        const prof = await apiProfile();
        if (prof?.success && prof?.data?.user) {
          u = prof.data.user;
        }
      } catch {
        // Si falla /profile, seguimos con el user del login
      }
